...
function createResizeObserver(ctx) {
  return new ResizeObserver(() => {
    ...
    ctx.dragOperation.shape = new geometry.Rectangle(
...
render_fn = function() {
  ...
  const elementStyles = utilities.getComputedStyles(element);
  const {
    transition,
    translate,
    boxSizing,
  ...
  const parsedTransform = utilities.parseTransform(elementStyles);
  ...
  const resizeObserver = createResizeObserver({
    ...
    styles,
    dragOperation,
    getTranslate: () => state$1.current.translate,
...
var _AutoScroller = class _AutoScroller extends abstract.Plugin {
  constructor(manager, options) {
    super(manager, options);
...
_AutoScroller.configure = abstract.configurator(_AutoScroller);
...
var DragDropManager = class extends abstract.DragDropManager {
  constructor(input = {}) {
    ...
    const modifiers = abstract.resolveCustomizable(
      input.modifiers,
...
var Draggable = class extends (_c = abstract.Draggable, _handle_dec = [state.reactive], _element_dec = [state.reactive], _c) {
